import { attributesFromFilter } from './aplu'
  ...
  describe('arbo', () => {
    describe('arbo', () => {
      scenario('arbo path', async (scenario) => {
        const root = await attributesFromFilter()
      ...
      scenario('arbo with filter', async () => {
        ...
        const root = await attributesFromFilter({ filter })
        const rootO = arboToObject<ArboDate>(root.arboDate)
    ...
    describe('tags', () => {
      scenario('returns tags without filter', async (scenario) => {
        const { tags } = await attributesFromFilter()
        expect(tags.length).toBe(5)
        expect(tags).toContainEqual(scenario.tag.g1t1)
        expect(tags).toContainEqual(scenario.tag.g1t2)
        expect(tags).toContainEqual(scenario.tag.g2t1)
        expect(tags).toContainEqual(scenario.tag.g2t2)
        expect(tags).toContainEqual(scenario.tag.g2t3)
      })

      scenario('returns tags with filter 1', async (scenario) => {
        const { tags } = await attributesFromFilter({
          filter: {
            rating: {
              value: 5,
